...
class CrossAttentionInjection(nn.Module):
    ...
    def forward(self, smri_features, fmri_keys, fmri_values):
        B, C, D, H, W = smri_features.shape
        
        downsample_factor = 4
        
        smri_downsampled = torch.nn.functional.avg_pool3d(
            smri_features, 
            kernel_size=downsample_factor, 
            stride=downsample_factor
        )
        
        B, C, D_small, H_small, W_small = smri_downsampled.shape
        
        smri_seq = smri_downsampled.view(B, C, -1).permute(0, 2, 1)
        
        queries = self.q_proj(smri_seq)
        ...
        attn_output = attn_output.permute(0, 2, 1).view(B, C, D_small, H_small, W_small)
        
        attn_output_upsampled = torch.nn.functional.interpolate(
            attn_output, 
            size=(D, H, W), 
            mode='trilinear', 
            align_corners=False
        ...
        smri_features = self.norm1(smri_features.permute(0, 2, 3, 4, 1) + attn_output_upsampled.permute(0, 2, 3, 4, 1))
        smri_features = smri_features.permute(0, 4, 1, 2, 3)
        
        ffn_input = smri_features.permute(0, 2, 3, 4, 1)
        ffn_output = self.ffn(ffn_input)
        smri_features = self.norm2(ffn_input + ffn_output).permute(0, 4, 1, 2, 3)
...
class SimpleBFNBlock(nn.Module):
    def __init__(self, in_channels, out_channels, prompt_dim, num_nodes=50, heads=4):
        super().__init__()
        self.num_nodes = num_nodes
        self.heads = heads
        self.input_proj = nn.Conv3d(in_channels, out_channels, 3, padding=1)

        self.node_pool = nn.AdaptiveAvgPool3d((num_nodes, 1, 1))

        self.encoder = GATEncoderBlock(out_channels, out_channels, num_nodes, heads=heads)

        self.film = FiLMLayer(out_channels, prompt_dim)

        self.decoder = GATDecoderBlock(out_channels, out_channels, fmri_dim=prompt_dim, num_nodes=num_nodes, heads=heads)
    ...
    def forward(self, x, prompt):
        B = x.shape[0]
        x = self.input_proj(x)
        x_nodes = self.node_pool(x)
        x_nodes = x_nodes.squeeze(-1).squeeze(-1).transpose(1,2)

        edge_index = self._build_full_conn_edge(self.num_nodes).to(x_nodes.device)

        h = self.encoder(x_nodes, edge_index)

        h_modulated = self.film(h, prompt)

        reconstructed = self.decoder(h_modulated, edge_index)

        energy = self.energy_head(reconstructed.transpose(1,2))
...
class SimplifiedHPBFN(nn.Module):
    def __init__(
        self,
    ...
    def compute_loss(self, outputs, t1w_target, y):
        ...
        fmri_features = outputs["fmri_features"]
        fmri_reg = torch.norm(fmri_features, p=2, dim=1).mean()
